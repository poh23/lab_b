...
def compute_SU_avg_distribution(m, wavelength, diameter_mean, diameter_std, angle_list, width, n_bins=30):
    """
    Computes SU(θ) averaged over a normal distribution of diameters.

    Parameters:
        m: relative refractive index
        wavelength: wavelength in nm
        diameter_mean: mean particle diameter in microns
        diameter_std: std dev of particle diameter in microns
        angle_list: list of angles to evaluate
        width: detector window width in degrees
        n_bins: number of size bins to evaluate

    Returns:
        averaged SU(θ) over the distribution
    """
    diameters = np.linspace(diameter_mean - 3*diameter_std, diameter_mean + 3*diameter_std, n_bins)
    weights = norm.pdf(diameters, loc=diameter_mean, scale=diameter_std)
    weights /= np.trapz(weights, diameters)  # Normalize PDF to unit area

    # Initialize result
    SU_total = np.zeros(len(angle_list))
    for d, w in zip(diameters, weights):
        SU = compute_SU_avg(m, wavelength, d, angle_list, width)
        SU_total += w * SU
...
def visualize_scattering_data(detector_width=5.0, lambda0 = 633, wavelength_error=5, n_particle=1.59, n_medium=1.33):
    """
    ...
    ax4.set_ylabel(r'Normalized $\frac{d\sigma}{d\Omega}$', fontsize=18)
    ...
    for i, diameter in enumerate(diameters):
        ...
        SU0 = compute_SU_avg_distribution(m, lambda0, diameter, diameter_error*2, angles, detector_width)

        # Numerical derivatives
        dl = wavelength_error

        SU_plus_l = compute_SU_avg_distribution(m, lambda0 + dl, diameter, diameter_error*2, angles, detector_width)
        SU_minus_l = compute_SU_avg_distribution(m, lambda0 - dl, diameter, diameter_error*2, angles, detector_width)
        df_dlambda = (SU_plus_l - SU_minus_l) / (2 * dl)

        # Total propagated uncertainty
        delta_SU = df_dlambda * dl
        ...
        if i < 3:  # We have 3 diameter-specific subplots
            ...
            ax.plot(angles, SU0_norm, 'k-', label=r'Theoretical')
            ...
            ax.set_ylabel(r'Normalized $\frac{d\sigma}{d\Omega}$', fontsize=18)
            ax.set_xlim(0, 130)
            ax.set_ylim(0, 1.5)
    ...
    plt.savefig(f'scattering_data_width_{detector_width}_w_pdf.pdf')
    plt.show()
