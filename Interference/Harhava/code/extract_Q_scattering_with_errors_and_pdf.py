...
# === 4. Calculate theoretical Qsca using a diameter PDF ===
def compute_Q_theory_pdf(row, wl=None, n_bins=50):
    """
    ...
    d_mean = row["diameter"] * 1e3             # μm → nm
    d_sigma = row["diameter_error_um2"] * 1e3  # μm → nm
    # if sigma is zero, fall back to single value
    if d_sigma == 0:
        return PMS.MieQ(m=m_rel, wavelength=row["wavelength"], diameter=d_mean)[1]
    # sample diameters ±3σ
    diameters = np.linspace(d_mean - 3*d_sigma,
                            d_mean + 3*d_sigma,
                            n_bins)
    # compute the PDF at each diameter
    if wl is None:
        wl = row["wavelength"]
...
def compute_Q_theory_error(row, n_bins=50):
    ...
    Q_wl_up = compute_Q_theory_pdf(row, wl_up, n_bins=n_bins)
    Q_wl_down = compute_Q_theory_pdf(row, wl_down, n_bins=n_bins)
    dQ_dwl = (Q_wl_up - Q_wl_down) / (2 * wavelength_error_nm)

    # Combine uncertainties
    Q_err = dQ_dwl * wavelength_error_nm
    return Q_err
...
df["Q_theory"] = df.apply(compute_Q_theory_pdf, axis=1)
